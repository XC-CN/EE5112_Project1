#!/usr/bin/env python3
"""Llama-4-Scout GPU dialogue system built on top of the local LLM platform."""
...
class DialogueSystem:
    """High-level Llama-4-Scout dialogue loop with conversation persistence."""
    ...
    def _load_config(self, path: Path) -> Dict[str, Any]:
        try:
            ...
            print(f"⚠️  Config file '{path}' not found. Using Llama-4-Scout platform defaults.")
            return LLMPlatform().config
        except json.JSONDecodeError as exc:
            print(f"⚠️  Invalid JSON in '{path}': {exc}. Using Llama-4-Scout platform defaults.")
            return LLMPlatform().config
        except Exception as exc:  # pragma: no cover - defensive
            print(f"⚠️  Failed to load config '{path}': {exc}. Using Llama-4-Scout platform defaults.")
            return LLMPlatform().config
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.platform.load_model():
                return False
            print("✅ Llama-4-Scout dialogue system initialised with RTX 5080 GPU acceleration.")
            return True
        except Exception as exc:  # pragma: no cover - defensive logging
            print(f"❌ Failed to initialise Llama-4-Scout dialogue system: {exc}")
            return False
    ...
    def chat(self, user_input: str) -> str:
        if not self.platform:
            return "Error: Llama-4-Scout platform not initialised. Call initialize() first."
    ...
    def show_conversation_stats(self) -> None:
        if not self.platform:
            print("Llama-4-Scout platform not initialised.")
            return

        info = self.platform.get_model_info()
        print("\n📊 Llama-4-Scout Conversation Statistics")
        print(f"Model: {info.get('model_name', 'Llama-4-Scout-17B-16E-Instruct')}")
        print(f"Model file: {Path(info['model_path']).name}")
        print(f"Messages in buffer: {info['conversation_length']}")
        print(f"GPU optimization: {info.get('gpu_optimized', 'RTX 5080 16GB')}")
        print(f"Context size: {info['model_config'].get('n_ctx', 8192)} tokens")
    ...
    def interactive_mode(self) -> None:
        ...
        print("\n🚀 Llama-4-Scout Dialogue System Ready (RTX 5080 GPU Mode)")
        print("🤖 Specialized in Robotics & Human-Robot Interaction")
        ...
        while True:
            try:
                ...
                if user_input.lower() == "exit":
                    print("👋 Thanks for using Llama-4-Scout! Goodbye!")
                    break
                ...
                if streaming_enabled:
                    print("🚀 Llama-4-Scout: ", end="", flush=True)
                ...
                print("\n👋 Interrupted by user. Thanks for using Llama-4-Scout!")
                break
...
def main(argv: Optional[Any] = None) -> None:
    parser = argparse.ArgumentParser(description="Llama-4-Scout GPU-accelerated dialogue system")
