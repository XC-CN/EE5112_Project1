...
$cacheDir = Join-Path $scriptDir "hf_cache"
...
if (-not $SkipConda) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Failed to activate conda environment" }
    }
    catch {
...
if ($Image) {
    ...
    Write-Host "未指定图片，将在运行时列出 data 目录内容" -ForegroundColor Cyan
}

if ($Save) { $arguments += '--save' }

Write-Host "=============================================="
Write-Host "启动 LLaVA 1.5 7B (4-bit) 多轮对话" -ForegroundColor Cyan
Write-Host "Python  : $pythonExe"
Write-Host "脚本    : $chatScript"
if ($Image) { Write-Host "图片    : $((Resolve-Path $Image).Path)" }
Write-Host "=============================================="
